...
class TestCLI:
    ...
    def test_parser_creation(self):
        ...
        subparsers_action = next(
            (
                ...
                if isinstance(action, type(parser._get_positional_actions()[0])) and action.dest == "command"
            ),
            None,
        )
    ...
    def test_train_command_args(self, mock_run):
        ...
        parser = build_parser()
        args = parser.parse_args(["train", "data.csv", "--target", "churn", "--problem-type", "classification"])
